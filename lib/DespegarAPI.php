                           ...
                           foreach ($item2 as $key3 => $item3){  // keys are count of array - Hotel data;
                                ...
                                foreach ($item3 as $node => $data){   // Keys for hotel information
                                    ...
                                    if (strpos( $node, 'Basedon value') !== FALSE){
                                          $hotelInfo["hotelReview"][$hotelCount] = $data[0]["text"];   // Hotel Rating
                                    }
...
//--------------------------------------------------------
     $result = query("7d0e8dbf-1b00-4737-af34-3507f6ae19b7", $apiKey ,"http://www.despegar.com/search/Hotel/265/2017-02-09/2017-02-10/2?from=SB#sorting=best_selling_descending&page=1&view=list");
     //var_dump($result);
 }
